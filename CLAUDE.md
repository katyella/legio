...
**Your Claude Code session IS the orchestrator.** There is no separate daemon. CLAUDE.md + hooks + the `legio` CLI provide everything.
...
legio/                        # This repo (the legio tool itself)
  src/
    ...
    commands/                     # One file per CLI subcommand
      agents.ts                   # legio agents (discover)
      init.ts                     # legio init
      sling.ts                    # legio sling (spawn worker)
      prime.ts                    # legio prime
      status.ts                   # legio status
      ...
      nudge.ts                    # legio nudge (tmux text nudge)
      merge.ts                    # legio merge
      spec.ts                     # legio spec write
      group.ts                    # legio group create/status/add/remove/list
      clean.ts                    # legio clean (nuclear cleanup)
      doctor.ts                   # legio doctor (health checks)
      worktree.ts                 # legio worktree list/clean
      ...
      replay.ts                   # legio replay (multi-agent replay)
      run.ts                      # legio run list/show/complete
    ...
    doctor/                       # Modular health check system
      *.ts                        # 9 check categories (see `legio doctor --help`)
...
- All error types go in `src/errors.ts` and must extend `LegioError` base class
...
legio nudge <agent> [message]       Send a text nudge to an agent via tmux
...
```
legio group <sub>                  Batch coordination
...
legio inspect <agent>               Deep inspection of a single agent
...
legio trace <target>               Chronological event timeline for agent/bead
...
legio replay                        Interleaved chronological replay across agents
  --run <id>  --agent <name>             Filter by run or agent (repeatable)
  --since <ts>  --until <ts>             Time range filter (ISO 8601)
  --limit <n>                            Max events (default: 200)
  --json                                 JSON output

legio run [sub]                     Manage runs (coordinator session groupings)
...
```
legio hooks <sub>                  Manage orchestrator hooks
