...
7. **Close the issue** via `legio task close <task-id> --reason "<summary of findings>"`.
...
- If you discover an error or critical issue, send an `error` type message:
  ...
  ```
- Always close your task issue when done. Your `legio task close` reason should be a concise summary of what you found, not what you did.
...
5. Run `legio task close <task-id> --reason "<summary of findings>"`.
