...
import type { AgentSession, LegioConfig } from "../types.ts";
 ...
 *
 * @param config - Legio configuration
 * @param legioDir - Absolute path to .legio/
...
export async function checkConsistency(
	...
	const worktreeBasePath = realpathSync(join(legioDir, "worktrees"));
	const legioWorktrees = worktrees.filter((wt) => wt.path.startsWith(worktreeBasePath));
	...
	const orphanedWorktrees = legioWorktrees.filter((wt) => !storeWorktreePaths.has(wt.path));
	...
	const legioTmuxPrefix = `legio-${projectName}-`;
	const legioTmuxSessions = tmuxSessions.filter((s) => s.name.startsWith(legioTmuxPrefix));
	const storeTmuxNames = new Set(storeSessions.map((s) => s.tmuxSession));

	const orphanedTmux = legioTmuxSessions.filter((s) => !storeTmuxNames.has(s.name));
