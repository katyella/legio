 ...
 */
export const checkConfig: DoctorCheckFn = async (config, legioDir): Promise<DoctorCheck[]> => {
	const checks: DoctorCheck[] = [];

	// Check 1: config-parseable
	const parseableCheck = await checkConfigParseable(legioDir);
	checks.push(parseableCheck);

	// Check 2: config-valid
	const validCheck = await checkConfigValid(legioDir);
 ...
 */
async function checkConfigParseable(legioDir: string): Promise<DoctorCheck> {
	try {
		// Try to load config - if this succeeds, config is parseable
		await loadConfig(legioDir);
 ...
 */
async function checkConfigValid(legioDir: string): Promise<DoctorCheck> {
	try {
 ...
 */
function checkProjectRootExists(config: LegioConfig): DoctorCheck {
 ...
 */
async function checkCanonicalBranchExists(config: LegioConfig): Promise<DoctorCheck> {
