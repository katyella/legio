 ...
 */
export const checkAgents: DoctorCheckFn = async (_config, legioDir): Promise<DoctorCheck[]> => {
	const checks: DoctorCheck[] = [];

	// Check 1: Parse agent-manifest.json
	const { manifest, errors: parseErrors } = await loadAndValidateManifest(legioDir);
