...
describe("createMergeQueue", () => {
	...
	beforeEach(async () => {
		tempDir = await mkdtemp(join(tmpdir(), "legio-merge-queue-test-"));
	...
	function makeInput(
		...
		return {
			branchName: overrides?.branchName ?? "legio/test-agent/bead-123",
	...
	describe("enqueue", () => {
		...
		test("returns the created entry with enqueuedAt timestamp", () => {
			...
			expect(entry.branchName).toBe("legio/test-agent/bead-123");
		...
		test("preserves all input fields on the returned entry", () => {
			...
			const input = makeInput({
				branchName: "legio/builder-1/bead-xyz",
				beadId: "bead-xyz",
				agentName: "builder-1",
			...
			expect(entry.branchName).toBe("legio/builder-1/bead-xyz");
