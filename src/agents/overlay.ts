...
function formatCanSpawn(config: OverlayConfig): string {
	...
	return [
		"You may spawn sub-workers using `legio sling`. Example:",
		"",
		"```bash",
		"legio sling <task-id> --capability builder --name <worker-name> \\",
 ...
 * `.claude/CLAUDE.md`, breaking the user's own Claude Code session (legio-uwg4).
 *
 * Uses deterministic path comparison instead of checking for `.legio/config.yaml`
 * because when dogfooding (running legio on its own repo), that file is tracked
 * in git and appears in every worktree checkout (legio-p4st).
 *
...
export async function writeOverlay(
	...
	// dogfooding scenarios where .legio/config.yaml is tracked in git
	// and appears in every worktree checkout (legio-p4st).
