/**
 * Tests for `legio trace` command.
 *
 * Uses real better-sqlite3 (temp files) to test the trace command end-to-end.
...
describe("traceCommand", () => {
	...
	describe("help flag", () => {
		test("--help shows help text", async () => {
			...
			expect(out).toContain("legio trace");
	...
	describe("target resolution", () => {
		...
		test("bead ID pattern is detected and resolved to agent name via SessionStore", async () => {
			...
			const sessDbPath = join(tempDir, ".legio", "sessions.db");
			...
			await traceCommand(["legio-rj1k", "--json"]);
			const out = output();
