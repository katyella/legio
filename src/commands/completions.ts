/**
 * Shell completion generation for legio CLI.
 *
...
export const COMMANDS: readonly CommandDef[] = [
	...
	{
		name: "init",
		desc: "Initialize .legio/ in current project",
		flags: [
	...
	{
		name: "doctor",
		desc: "Run health checks on legio setup",
		flags: [
			{ name: "--json", desc: "JSON output" },
	...
	},
	{
		...
		subcommands: [
			{
				name: "start",
				desc: "Start the local web UI server",
				flags: [
					{ name: "--port", desc: "Port to listen on", takesValue: true },
					{ name: "--host", desc: "Bind address", takesValue: true },
					{ name: "--open", desc: "Auto-open browser" },
					{ name: "--help", desc: "Show help" },
				],
			},
		],
...
export function generateBash(): string {
	const lines: string[] = [
		"# Bash completion for legio",
		"# Source this file to enable completions:",
		"#   source <(legio --completions bash)",
		"",
		"_legio() {",
	...
	lines.push("}");
	lines.push("");
	lines.push("complete -F _legio legio");
...
export function generateZsh(): string {
	const lines: string[] = [
		"#compdef legio",
		"# Zsh completion for legio",
		"# Place this file in your fpath or source it:",
		"#   source <(legio --completions zsh)",
		"",
		"_legio() {",
		"  local -a commands",
		"  commands=(",
...
export function generateFish(): string {
	const lines: string[] = [
		"# Fish completion for legio",
		"# Place this file in ~/.config/fish/completions/legio.fish or source it:",
		"#   legio --completions fish | source",
		"",
		"# Remove all existing completions for legio",
		"complete -c legio -e",
		"",
		"# Global options",
		"complete -c legio -l help -d 'Show help'",
		"complete -c legio -l version -d 'Show version'",
		"complete -c legio -l completions -d 'Generate shell completions' -xa 'bash zsh fish'",
		"",
	];
	...
	for (const cmd of COMMANDS) {
		...
		if (cmd.subcommands && cmd.subcommands.length > 0) {
			...
			for (const subcmd of cmd.subcommands) {
				lines.push(
					`complete -c legio -f -n '__fish_seen_subcommand_from ${cmd.name}; and not __fish_seen_subcommand_from ${cmd.subcommands.map((s) => s.name).join(" ")}' -a '${subcmd.name}' -d '${subcmd.desc}'`,
				);
				...
				if (subcmd.flags && subcmd.flags.length > 0) {
					for (const flag of subcmd.flags) {
						...
						if (flag.values) {
							lines.push(
								`complete -c legio -f -n ${cond} -l '${flagName}' -d '${flag.desc}' -xa '${flag.values.join(" ")}'`,
							);
						} else if (flag.takesValue) {
							lines.push(`complete -c legio -n ${cond} -l '${flagName}' -d '${flag.desc}'`);
						} else {
							lines.push(`complete -c legio -f -n ${cond} -l '${flagName}' -d '${flag.desc}'`);
						}
					}
				}
			}
...
export function completionsCommand(args: string[]): void {
	...
	if (!shell) {
		...
		process.stderr.write("Usage: legio --completions <bash|zsh|fish>\n");
		process.exit(1);
	}
