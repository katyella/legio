...
beforeEach(async () => {
	tempDir = await mkdtemp(join(tmpdir(), "legio-run-test-"));
...
async function readCurrentRunFile(): Promise<string | null> {
	const file = fileAccessor(join(legioDir, "current-run.txt"));
...
describe("edge cases", () => {
	...
	test("sessions.db does not exist for list returns empty", async () => {
		...
		const file = fileAccessor(dbPath);
		expect(await file.exists()).toBe(false);

		// Re-create stores for afterEach cleanup
		runStore = createRunStore(join(legioDir, "unused-run.db"));
		sessionStore = createSessionStore(join(legioDir, "unused-session.db"));
	});
});
