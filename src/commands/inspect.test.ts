/**
 * Tests for `legio inspect` command.
 *
 * Uses real better-sqlite3 (temp files) to test the inspect command end-to-end.
...
describe("inspectCommand", () => {
	...
	describe("help flag", () => {
		test("--help shows help text", async () => {
			...
			const out = output();
			expect(out).toContain("legio inspect");
	...
	describe("validation", () => {
		...
		test("throws if --interval is invalid", async () => {
			...
			store.upsert({
				...
				capability: "builder",
				worktreePath: "/tmp/wt",
				branchName: "legio/builder-1/test",
				beadId: "legio-001",
	...
	describe("gatherInspectData", () => {
		test("gathers basic session data", async () => {
			...
			expect(data.session.beadId).toBe("legio-001");
	...
	describe("human-readable output", () => {
		test("displays agent metadata", async () => {
			...
			expect(out).toContain("legio-001");
			expect(out).toContain("builder");
			expect(out).toContain("legio/builder-1/test");
