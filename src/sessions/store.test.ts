...
beforeEach(async () => {
	tempDir = await mkdtemp(join(tmpdir(), "legio-sessions-test-"));
...
describe("upsert", () => {
	...
	test("all fields roundtrip correctly (camelCase TS -> snake_case SQLite -> camelCase TS)", () => {
		const session = makeSession({
			...
			branchName: "legio/roundtrip-agent/task-42",
			beadId: "task-42",
			tmuxSession: "legio-roundtrip-agent",
			state: "working",
			pid: 99999,
...
describe("edge cases", () => {
	...
	test("special characters in agent name and worktree path", () => {
		const session = makeSession({
			...
			branchName: "legio/agent-with-special_chars.v2/task-1",
		});
