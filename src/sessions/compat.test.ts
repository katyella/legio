...
describe("openSessionStore", () => {
	...
	test("does not re-migrate when sessions.db already exists", async () => {
		...
		const { store: store1, migrated: migrated1 } = openSessionStore(legioDir);
		...
		const { store: store2, migrated: migrated2 } = openSessionStore(legioDir);
	...
	test("imports from sessions.json when sessions.db exists but is empty", async () => {
		...
		// a previous coordinator start that used the old code path (legio-036f).
		const { store: emptyStore } = openSessionStore(legioDir);
...
describe("data integrity", () => {
	test("all fields from sessions.json are preserved in SQLite", async () => {
		const fullSession = makeJsonSession({
			...
			branchName: "legio/full-agent/task-42",
			beadId: "task-42",
			tmuxSession: "legio-full-agent",
			state: "stalled",
			pid: 99999,
		...
		expect(result?.branchName).toBe("legio/full-agent/task-42");
		expect(result?.beadId).toBe("task-42");
		expect(result?.tmuxSession).toBe("legio-full-agent");
